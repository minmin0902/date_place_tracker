...
create table if not exists public.foods (
  ...
  name text not null,
  my_rating numeric check (my_rating is null or (my_rating >= 0 and my_rating <= 5)),
  partner_rating numeric check (partner_rating is null or (partner_rating >= 0 and partner_rating <= 5)),
  category text,
  memo text,
  photo_url text,
  created_at timestamptz not null default now()
